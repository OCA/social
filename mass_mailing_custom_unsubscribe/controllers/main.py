...
class CustomUnsubscribe(MassMailController):
    def _mailing_unsubscribe_from_list(self, mailing, document_id, email, hash_token):
        ...
        # We need to for the unsubscription as the context won't get to the method
        # from here
        mailing.contact_list_ids.with_context(
            **request.env.context
        )._track_mail_unsubscription(
            email, "unsubscription", list_ids=mailing.contact_list_ids
        )
