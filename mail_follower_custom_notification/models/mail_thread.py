...
class MailThread(models.AbstractModel):
    ...
    def message_custom_notification_update_user(self, custom_notifications):
        ...
        user2partner = dict(
            self.env["res.users"]
            .browse(map(int, custom_notifications.keys()))
            ...
            follower.write(
                {
                    "force_mail_subtype_ids": [
                        (6, 0, ids_with_value(data, "force_mail", "force_yes"))
                    ],
                    "force_nomail_subtype_ids": [
                        (6, 0, ids_with_value(data, "force_mail", "force_no"))
                    ],
                    "force_own_subtype_ids": [
                        (6, 0, ids_with_value(data, "force_own", "1"))
                    ],
                }
            ),
