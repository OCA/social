...
- **Parent Stage Field**: Choose the many2many field from the model of
  ...
  field. If the Reply Stage is not present in the Parent Stage Field, it
  will not be assigned to the record.
- **domain**: Set a domain to filter which records this config applies
  to. Example: `[('project_id.name', '=', 'My Project')]`
- **Reply Stage Field**: Choose the field (e.g., stage_id) to be updated
  when a non-internal user replies. (required)
