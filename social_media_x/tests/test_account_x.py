...
class TestSocialAccountX(TestSocialCommonX):
    ...
    def test_create_account_x(self):
        ...
        with patch_get_client_api as mock_get_client_api, patch(
            ...
            self.assertTrue(
                ...
                msg="The initial sync only targets the associated account.",
            )
    ...
    def test_update_posts_statistics_since_keeps_account_totals(self):
        """A page read from ``since_id`` must not replace the account totals.

        With *Enable since* the timeline only carries what was published after
        the stored publication, so aggregating it would leave the account with
        the figures of those few posts instead of its own.
        """
