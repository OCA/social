...
class TestSocialCommonX(TestSocialMediaBaseCommon):
    ...
    def create_attachment(self, attach_name="test_image.jpg", size=None):
        ...
        """
        content = b"\0" * size if size else b"testmedia"
    ...
    def get_patch_super_x(record, module_class, method, **kwargs):
        """Patch ``method`` on the class ``module_class`` delegates to.

        ``record.__class__`` is the registry class Odoo builds per model, and
        its MRO chains one class per installed module. Locating the X class
        and walking forward to the first class actually defining ``method``
        patches the ``super()`` the X override delegates to: the class right
        after X is not necessarily the one providing it, since a module such
        as ``social_media_advertising_linkedin`` extends the same model
        without overriding every method.
        ...
        for parent in mro[mro.index(module_class) + 1 :]:
            if method in parent.__dict__:
                return patch.object(parent, method, **kwargs)
        raise AssertionError(  # pragma: no cover
            f"No parent of {module_class.__name__} defines '{method}'"
        )
