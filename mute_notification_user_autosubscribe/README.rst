...
.. 
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   !! source digest: sha256:2e2c22345524e35ac4f687ef2b3d51a32257f1ecef09976aec48408a9463012b
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
- Create a new instance or edit an exiting one. The following fields are
  available:
  ...
  - **Users**. List of users which the rule will be applied to. The
    users in this field will be autosubscribed to the document when they
    are set in the user_id field but only with the "Muted" subtype,
    which notifies nothing. Note that this rule will not be applied when
    the user in the list set itself in the user_id field, as the
  ...
  - **Groups**. List of groups which the rule will be applied to. The
    users that belong to the groups in this field will be autosubscribed
    to the document when they are set in the user_id field but only with
    the "Muted" subtype, which notifies nothing. Note that this rule
    will not be applied when a user in any of the groups in the list set
    itself in the user_id field, as the subscription in this case
    depends on the creation of the document, not the autosubscription.
  - **Notes**. Text field.
