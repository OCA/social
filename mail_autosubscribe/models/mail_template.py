...
class MailTemplate(models.Model):
    ...
    def _generate_template_recipients(self, res_ids, render_fields,
                                      ...
                                      render_results=None):
        res = super()._generate_template_recipients(res_ids, render_fields, find_or_create_partners, render_results)
        ...
        if autosubscribe_followers:
            for res_val in res.values():
                if "partner_ids" in res_val:
                    ...
                    partners = self.env["res.partner"].sudo().browse(res_val["partner_ids"])
                    followers = ResModel._message_get_autosubscribe_followers(partners)
                    follower_ids = [follower.id for follower in followers if follower not in partners]
