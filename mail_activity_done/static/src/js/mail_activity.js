        ...
        _.each(activities, function ( activity ) {
            ...
            if (diff === 0) {
                ...
                if (diff === -1) {
                    ...
                    to_display = _.str.sprintf(_t('%d days overdue'),
                        Math.abs(diff));
                }
            } else if (diff === 1) {
                // Due
                ...
                to_display = _.str.sprintf(_t('Due in %d days'),
                    Math.abs(diff));
            }
    ...
    mail_activity.include({
        _render: function () {
            _.each(this._activities, function ( activity ) {
                var note = mailUtils.parseAndTransform(activity.note || '',
                    mailUtils.inline);
                var is_blank = (/^\s*$/).test(note);
                if (is_blank) {
                    activity.note = '';
                } else {
                    activity.note = mailUtils.parseAndTransform(activity.note,
                        mailUtils.addLink);
                }
            });
