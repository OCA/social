...
class ir_mail_server(models.Model):
    ...
    def test_imap_connection(self):
        ...
        try:
            ...
            raise ValidationError(_("Connection Test Failed!"), _("Here is what we got instead:\n %s") % tools.ustr(e))
        finally:
            try:
                if result: maillib.logout()
            except Exception:
    ...
    def send_email(self, message, mail_server_id=None, smtp_server=None, smtp_port=None, smtp_user=None, smtp_password=None, smtp_encryption=None, smtp_debug=False):
        result = super(ir_mail_server, self).send_email(message, mail_server_id, smtp_server, smtp_port, smtp_user, smtp_password, smtp_encryption, smtp_debug)
    ...
    def _save_sent_message_to_sentbox(self, msg, mail_server_id,):
        ...
        mail_server = None
        mail_server = ir_mail_server.sudo().search([], order='sequence', limit=1)
        if mail_server:
        ...
        if not smtp_server:
            raise ValidationError(("Please define at least one SMTP server, or provide the SMTP parameters explicitly."))
        maillib = imaplib.IMAP4_SSL(smtp_server)
        try:
            maillib.login(smtp_user, smtp_password)
            folder = mail_server.imap_mailbox_folder.name.join('""')
            res = maillib.append(folder, '\Seen', None, str(msg))
        except Exception, ex:
                _logger.exception('failed attaching mail via imap to server %s %s' % (ex ,msg))
        finally:
            maillib.logout()
        return True

    imap_mailbox_folder = fields.Many2one('ir.mail.imap.folder', 'Imap Folders', )
