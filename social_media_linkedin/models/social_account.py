...
class SocialAccount(models.Model):
    ...
    refresh_token_expires_in = fields.Date()
    ...
    def _refresh_token(self):
        ...
        if isinstance(response, dict):
            ...
            raise ValidationError(_("Error refreshing token: %s") % response.text)
    ...
    def _prepare_url_upload_asset(self, feedshare="image"):
        try:
            ...
            if not isinstance(asset, dict):
                raise ValidationError(_("Error uploading video: %s") % asset.text)
            else:
            ...
            _logger.error(e)
            raise ValidationError(_("Error uploading video")) from e
    ...
    def get_account_linkedin(self, access_token):
        ...
        for organization_id in organization_ids:
            ...
            if (
                ...
                complete_url = list(
                    filter(lambda x: "logo_400_400" in x.get("artifact", ""), elements)
                )
            ...
            localized_name = response_organizations.get("name", {}).get("localized", {})
        ...
        if isinstance(token, dict):
            ...
            if access_token:
                ...
                social_account_ids = list(
                    map(
                        lambda x: x["linkedin_account_id"],
                        self.env["social.account"]
                        .sudo()
                        .search_read([], ["linkedin_account_id"]),
                    )
                )
                for organization in organizations:
                    ...
                    if str(organization.get("id", False)) not in social_account_ids:
                        linkedin_account_urn = (
    ...
    def validate_access_token(self):
        ...
        if (
            ...
            if self.expire_access_token_date < datetime.now(tz=timezone).date():
        ...
        try:
            for account in account_ids:
                ...
                if account.linkedin_account_id:
                    ...
                    # POSTS REACTIONS
                    post_reactions = account._get_entity_share_statistics(
                        posts=ugc_posts
                    )
                    ...
                    for ugc_post in ugc_posts:
                        ...
                        data = {
                            ...
                            "engagement": round(post_data_reaction[4], 2)
                            if post_data_reaction
                            else 0,
                        ...
                        update_account_data.update(
                            {
                                ...
                                "comment_count": share_statistics.get(
                                    "commentCount", 0
                                ),
        ...
        for account in account_ids:
            ...
            if account_share_statistics:
                ...
                impression_count = sum(
                    [
                        val.get("totalShareStatistics", {}).get("impressionCount", 0)
                        for val in account_share_statistics
                    ]
                )
                comment_count = sum(
                    [
                        val.get("totalShareStatistics", {}).get("commentCount", 0)
                        for val in account_share_statistics
                    ]
                )
                reaction_count = sum(
                    [
                        val.get("totalShareStatistics", {}).get("likeCount", 0)
                        + val.get("totalShareStatistics", {}).get("shareCount", 0)
                        for val in account_share_statistics
                    ]
                )
                data_linkedin.append(
                    {
                        "id": account.id,
                        "name": _(f"[{account.media_type.upper()}] {account.name}"),
                                ...
                                post_account = PostAccount.search_count(
                                    [
                                        ...
                                        (
                                            "comment_count",
                                            "!=",
                                            statistic.get("commentCount", 0),
                                        ),
                                        "|",
                                        (
                                            "like_count",
                                            "!=",
                                            statistic.get("likeCount", 0),
                                        ),
                                        "|",
                                        (
                                            "click_count",
                                            "!=",
                                            statistic.get("clickCount", 0),
                                        ),
                                        (
                                            "share_count",
                                            "!=",
                                            statistic.get("shareCount", 0),
                                        ),
                                    ]
                                )
                                if post_account:
