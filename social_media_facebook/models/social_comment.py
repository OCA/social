...
class SocialComment(models.Model):
    """Feature #4: Comment Moderation System"""
    ...
    reply_count = fields.Integer(string="Reply Count", compute="_compute_reply_count", store=True)
    ...
    def action_hide(self):
        """Hide comment on social platform"""
        self.ensure_one()
        # Find Facebook account associated with this post
        account = self.post_id.account_ids.filtered(lambda a: a.media_type == "facebook")[:1]
        if account:
            success = account._hide_facebook_comment(self.comment_id)
            if success:
                self.is_hidden = True
        return True
