...
- *Fetch campaigns*, and *Create in LinkedIn* on a campaign or a campaign
  group that is not on LinkedIn yet, work against the advertising account
  marked *In Use*.
- What already exists on LinkedIn keeps the advertising account it was
  created in or imported from, whichever one is in use later: updating,
  archiving or checking a campaign or a campaign group, and deleting an ad,
  address the advertising account stored on the record.
- A new campaign whose group already exists on LinkedIn is created in the
  advertising account of that group, and the sponsored creative of a post is
  created in the advertising account of its campaign.
- A record that exists on LinkedIn but lost its advertising account, because
  that advertising account is no longer returned by LinkedIn, is never sent
  to the one in use instead. Updating, archiving or checking such a campaign
  or campaign group, creating a campaign in such a group and deleting such an
  ad are refused with *The advertising account of ... is no longer available
  on LinkedIn.* A post sponsored with such a campaign is published anyway:
  its creative cannot be created, and the failure is reported on the chatter
  of the post.
...
- If the campaign cannot be created, the error is shown in a notification
  ...
  is reused, and the campaign is created in its advertising account.
...
- A group has no LinkedIn account of its own: to create, update or archive
  it on LinkedIn, Odoo takes the LinkedIn account from the campaigns of the
  ...
  its campaigns first. A new group is created in the advertising account in
  use of that LinkedIn account, and a group already on LinkedIn is updated
  and archived in the advertising account stored on it.
...
- The ad is deleted in the advertising account stored on it, not in the one
  in use. An ad whose advertising account is no longer returned by LinkedIn
  cannot be deleted from Odoo.
