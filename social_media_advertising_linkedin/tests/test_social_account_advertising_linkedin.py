...
class TestSocialAccountAdvertisingLinkedin(TestSocialCommonAdvertisingLinkedin):
    ...
    def test_get_advertising_account_without_one_in_use(self):
        ...
        with self.assertRaises(UserError) as error:
            account._require_linkedin_advertising_account()
    ...
    def test_advertising_hooks_ignore_another_social_media(self):
        """Outside LinkedIn every hook answers what the generic layer does."""
        account = self.social_account_id
        self.assertEqual(account._fetch_advertising_accounts(), [])
        self.assertEqual(account._fetch_ads(), [])
        self.assertEqual(account._fetch_ad_refs(), set())
        res = account.action_import_campaigns()
        self.assertFalse(res["success"])
        self.assertIn("not available for this social media", res["message"])

    def test_fetch_ad_account_reports_a_refusal(self):
        """An advertising account that cannot be read explains why."""
        account = self.SocialAccountLinkedin
        error_response = self.generate_magic_mock(
            **{"status_code": 400, "json_return_value": {"message": "Nope"}}
        )
        with self._mock_linkedin(error_response, account):
            with self.assertRaises(UserError) as error:
                account._fetch_linkedin_ad_account("urn:li:sponsoredAccount:999")
        self.assertIn("could not be read from LinkedIn", str(error.exception))

    def test_fetch_creatives_without_an_advertising_account(self):
        """Nothing of the Ads API is addressed without one, so nothing is called."""
        self.AdvertisingAccountLinkedin.write({"is_current": False})
        account = self.SocialAccountLinkedin
        with self._mock_linkedin(MagicMock(), account) as mock_request:
            self.assertEqual(account._fetch_linkedin_creatives(), [])
        mock_request.assert_not_called()

    def test_fetch_ad_entities_reports_a_refusal(self):
        """A page LinkedIn refuses stops the pagination with the reason."""
        account = self.SocialAccountLinkedin
        error_response = self.generate_magic_mock(
            **{"status_code": 400, "json_return_value": {"message": "Nope"}}
        )
        with self._mock_linkedin(error_response, account):
            with self.assertRaises(UserError) as error:
                account._fetch_linkedin_ad_entities("/adAccounts/999/adCampaigns")
        self.assertIn(
            "The campaigns could not be read from LinkedIn", str(error.exception)
        )

    def test_fetch_ad_entities_passes_the_search_and_the_fields(self):
        """What the finder filters and projects travels as query parameters."""
        account = self.SocialAccountLinkedin
        page = self.generate_magic_mock(
            **{"status_code": 200, "json_return_value": {"elements": [{"id": 1}]}}
        )
        with self._mock_linkedin(page, account) as mock_request:
            elements = account._fetch_linkedin_ad_entities(
                "/adAccounts/999/adCampaigns",
                search="(test:false)",
                fields="id,name",
            )
        ...
        self.assertIn("search", call_kwargs["params_fields"])
        self.assertIn("fields", call_kwargs["params_fields"])
        self.assertEqual(call_kwargs["params_values"]["search"], "(test:false)")
        self.assertEqual(call_kwargs["params_values"]["fields"], "id,name")

    def test_link_creatives_skips_what_it_cannot_match(self):
        """A creative of an unknown publication, or without one, links nothing."""
        linked = self.SocialAccountLinkedin._link_linkedin_creatives(
            [
                {"id": "urn:li:sponsoredCreative:1", "content": {}},
                {
                    "id": "urn:li:sponsoredCreative:2",
                    "content": {"reference": "urn:li:share:unknown"},
                },
            ]
        )
        self.assertEqual(linked, 0)

    def test_advertising_account_web_url_is_empty_outside_linkedin(self):
        """Only a LinkedIn one knows its address in the Campaign Manager."""
        advertising_account = self.env["social.advertising.account"].create(
            {
                "account_id": self.social_account_id.id,
                "name": "Generic Ads",
                "remote_ref": "generic-1",
                "environment": "test",
            }
        )
...
class TestSocialAccountAdsScopesLinkedin(TestSocialCommonAdvertisingLinkedin):
    ...
    def test_missing_ads_scopes_ignores_another_social_media(self):
        """An account of another social media is asked for no LinkedIn scope."""
        self.assertFalse(self.social_account_id.linkedin_missing_ads_scopes)
