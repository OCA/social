...
class TestSocialAdvertisingCampaignLinkedin(TestSocialCommonAdvertisingLinkedin):
    ...
    def test_publish_campaign_group_without_a_group(self):
        """A campaign outside a group has nothing to create it under."""
        campaign = self.SocialAdvertisingCampaign.create(
            {
                "name": "Groupless Campaign",
                "media_id": self.media_linkedin_data_id.id,
                "account_ids": [Command.link(self.SocialAccountLinkedin.id)],
            }
        )
        with self.assertRaises(UserError) as error:
            campaign._linkedin_publish_campaign_group(
                self.SocialAccountLinkedin, ADVERTISING_ACCOUNT_URN
            )
        self.assertIn("does not belong to a campaign group", str(error.exception))

    def test_verify_campaign_keeps_the_one_linkedin_answers(self):
        campaign = self.SocialAdvertisingCampaignLinkedin
        patch_request = self.get_patch_exceptions_linkedin(
            side_effect=[MagicMock(status_code=200)]
        )
        with patch_request:
            self.assertEqual(
    ...
    def test_verify_campaign_forgets_the_one_linkedin_lost(self):
        """A campaign LinkedIn does not know is created again, not updated."""
        campaign = self.SocialAdvertisingCampaignLinkedin
        patch_request = self.get_patch_exceptions_linkedin(
            side_effect=[MagicMock(status_code=404)]
        )
        with patch_request:
            self.assertFalse(
                campaign._linkedin_verify_campaign(self.SocialAccountLinkedin)
    ...
    def test_verify_campaign_reports_an_unexpected_answer(self):
        campaign = self.SocialAdvertisingCampaignLinkedin
        error_response = MagicMock(status_code=400)
        error_response.json.return_value = {"message": "Nope"}
        patch_request = self.get_patch_exceptions_linkedin(side_effect=[error_response])
        with patch_request:
            with self.assertRaises(UserError) as error:
                campaign._linkedin_verify_campaign(self.SocialAccountLinkedin)
        self.assertIn("could not be checked on LinkedIn", str(error.exception))

    def test_validate_publish_linkedin_lists_every_missing_piece(self):
        """The errors come out at once instead of one round trip at a time."""
        campaign = self.SocialAdvertisingCampaign.create({"name": "Bare Campaign"})
        with self.assertRaises(UserError) as error:
            campaign._validate_publish_linkedin()
        message = str(error.exception)
        self.assertIn("media must be LinkedIn", message)
        self.assertIn("must belong to a campaign group", message)
        self.assertIn("must have a social account", message)

    def test_action_update_linkedin_without_a_remote_reference(self):
        campaign = self.SocialAdvertisingCampaign.create(
            {
                "name": "Unsynced Campaign",
                "campaign_group_id": self.SocialAdvertisingCampaignGroupLinkedin.id,
                "media_id": self.media_linkedin_data_id.id,
                "account_ids": [Command.link(self.SocialAccountLinkedin.id)],
            }
    ...
    def test_action_update_linkedin_without_a_currency(self):
        group = self.SocialAdvertisingCampaignGroup.create(
            {"name": "Currencyless Group"}
        )
        campaign = self.SocialAdvertisingCampaign.create(
            {
                ...
                "media_id": self.media_linkedin_data_id.id,
                "account_ids": [Command.link(self.SocialAccountLinkedin.id)],
                "remote_ref": "urn:li:sponsoredCampaign:900",
            }
        )
        with self.assertRaises(UserError) as error:
            campaign.action_update_linkedin()
        self.assertIn("social account and a currency", str(error.exception))

    def test_action_update_linkedin_without_the_group_on_linkedin(self):
        """A campaign belongs to its group on LinkedIn too, so it must exist."""
        group = self.SocialAdvertisingCampaignGroup.create(
            {
                "name": "Local Group",
                "total_budget": 100,
        ...
        campaign = self.SocialAdvertisingCampaign.create(
            {
                "name": "Campaign of a local group",
                "campaign_group_id": group.id,
                "media_id": self.media_linkedin_data_id.id,
                "account_ids": [Command.link(self.SocialAccountLinkedin.id)],
                "remote_ref": "urn:li:sponsoredCampaign:901",
                "unit_cost": 1,
                "daily_budget": 10,
            }
        )
        with self.assertRaises(UserError) as error:
            campaign.action_update_linkedin()
        self.assertIn("does not exist on LinkedIn", str(error.exception))

    def test_action_update_linkedin_reports_a_refusal(self):
        """A refused update keeps the campaign flagged as pending."""
        campaign = self.SocialAdvertisingCampaignLinkedin
        campaign.write({"name": "Campaign 1 renamed"})
        self.assertTrue(campaign.linkedin_needs_update)
        error_response = MagicMock(status_code=400)
