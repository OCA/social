...
class TestSocialAdvertisingAdLinkedin(TestSocialCommonAdvertisingLinkedin):
    ...
    def test_remote_ad_hooks_ignore_another_social_media(self):
        """Outside LinkedIn the generic behaviour is what an ad gets."""
        ad = self.SocialAdvertisingAd.create(
            {
                "remote_ref": "generic-ad-1",
                "account_id": self.social_account_id.id,
            }
        )
        ad._register_remote_ad_gone()
        self.assertFalse(ad.active)
        self.assertFalse(ad.stage_id)
        with self.assertRaises(UserError) as error:
            ad._delete_remote_ad()
        self.assertIn("not available for this social media", str(error.exception))
